...
export default function NewDeal() {
  ...
  async function submit(formData: FormData) {
    ...
    const response = await fetch("/api/deals", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    const result = await response.json() as { deal?: { id: number }, error?: string };
    if (result.deal) location.href = `/deal/${result.deal.id}`;
    else setState(result.error || "The deal could not be created.");
  }
