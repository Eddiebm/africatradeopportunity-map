...
import { dealCosts, dealDocuments, dealEvents, deals, documentFiles, milestones, organizationMembers, organizations, quoteRequests, quotes, verificationChecks } from "../../../db/schema";
...
export default async function DealRoom({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="portal">
    ...
    <section className="roommetrics"><article><small>{quoteLanded ? "QUOTE-BACKED LANDED COST" : "ESTIMATED LANDED COST"}</small><b>${landed.toLocaleString()}</b></article><article><small>REPORTED SALE VALUE</small><b>${(cost?.expectedRevenue || 0).toLocaleString()}</b></article><article><small>{quoteLanded ? "QUOTE-BACKED PROFIT" : "ESTIMATED PROFIT"}</small><b className={profit >= 0 ? "positive" : "negative"}>${profit.toLocaleString()}</b></article><article><small>DECISION</small><b>{profit > 0 && score === 100 ? "REVIEW TO PROCEED" : "HOLD"}</b></article></section>
    ...
    <section className="roomgrid">
      <article style={{ gridColumn: "1/3" }}>
        ...
        <QuoteRequestForm dealId={id} myOrganizations={myOrganizations} />
        {dealQuoteRequests.length ? dealQuoteRequests.map((qr) => {
          ...
          return <div key={qr.id} style={{ borderTop: "1px solid #e1e2da", paddingTop: 10, marginTop: 10 }}>
            ...
            {quotesForRequest.map((q) => <div className="task" key={q.id}><i>$</i><span><b>{q.currency} {(q.goodsTotal + q.freightTotal + q.borderEstimate + q.inspectionTotal + q.insuranceTotal + q.financeFxTotal + q.otherTotal).toLocaleString()} total landed</b><small>{q.status} · valid until {q.validUntil} · {q.assumptions.slice(0, 120)}{q.assumptions.length > 120 ? "…" : ""}</small></span>{q.status === "submitted" && <QuoteActions quoteId={q.id} />}</div>)}
          </div>;
        }) : <p>No quote requests yet for this deal — request one above once you have a counterparty organization in mind.</p>}
      </article>
    </section>
    <section className="release"><div className="roomtitle"><small>PROPOSED PAYMENT RELEASES</small><b>Licensed partner execution required</b></div>{steps.map((step) => <article key={step.id}><i>{step.sequence}</i><span><b>{step.name}</b><small>{step.releaseCondition}</small></span><strong>{step.percentage}%</strong><MilestoneEvidenceButton dealId={id} milestoneId={step.id} evidenceStatus={step.evidenceStatus} /></article>)}</section>
